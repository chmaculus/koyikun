...
$array_usuario=array_usuario($id_usuario);

echo '<font1>Bienvenido '.$array_usuario["nombre"].'</font1><br><br>';
...
while($row=mysql_fetch_array($result)){
	...
	if($array_precios["promocion"]=="S"){
		...
		$time_inicio=strtotime($array_promo["fecha_inicio"]);
		$time_finaliza=strtotime($array_promo["fecha_finaliza"]);
		$time_fecha=strtotime($fecha);

		if($time_fecha >= $time_inicio){
		//	echo '$fecha >= $array_promo["fecha_inicio"]';
		}else{
		//	echo ' no se cumpliio $fecha >= $array_promo["fecha_inicio"]';
		}

		if($time_fecha <= $time_finaliza){
			//echo '$fecha <= $array_promo["fecha_finaliza"]';
		}else{
			//echo ' no se cumplio $fecha <= $array_promo["fecha_finaliza"]';
		}

		if($time_fecha >= $time_inicio AND $time_fecha <= $time_finaliza AND $array_promo["habilitado"]=="S"){
			//es promo
	...
	if($array_stock2["stock"]<0){
		$stock2=0;
	}else{
		$stock2=$array_stock2["stock"];
	}

	//if(file_exists ( "./imagenes/miniatura/".$row["id"].".jpg" )==1){
	//}
...
//	echo '<td>'.$array_precios["query"].'</td>';
	echo '<td>'.$contado.'</td>';
	echo '<td>'.$tarjeta.'</td>';
	echo '<td>'.$stock.'</td>'.chr(13);
	echo '<td>'.$stock2.'</td>'.chr(13);
	...
	//////////////////////
	if($array_usuario["vender"]!="NO"){
		echo '<td><A HREF="../sucursal_ventas/articulo_vender.php?id_articulo='.$row["id"].'"><button>Vender</button></A></td>';
	}else{
		echo "<td></td>";
	}
	
	echo '<td><A HREF="tarjetas_listado.php?id_articulo='.$row["id"].'" onClick="return popup(this, \'notes\')"><button>Tarjetas</button></A></td>';
	echo '<td><A HREF="stock_sucursales.php?id_articulo='.$row["id"].'" onClick="return popup(this, \'notes\')"><button>stock</button></A></td>';
	echo '<td><A HREF="listas.php?id_articulo='.$row["id"].'" onClick="return popup(this, \'notes\')"><button>Listas</button></A></td>';
	echo '<td><A HREF="../sucursal_ventas/articulo_pedir.php?id_articulo='.$row["id"].'" onClick="return popup(this, \'notes\')"><button>Pedir</button></A></td>';
	//////////////////////
	echo "</tr>".chr(10);
}
echo "</table>";
...
#devuelve 4 columnas en la tabla
	...
	echo '<td><A HREF="../sucursal_ventas/articulo_pedir.php?id_articulo='.$id_articulo.'" onClick="return popup(this, \'notes\')"><button>Pedir</button></A></td>';
}
#-----------------------------------------------------------------
