...
if($_POST["accion"]=="ACTUALIZAR"){
	...
	while($row=mysql_fetch_array($result)){
		...
		if($cantidad<"1"){
			$query2='delete from pedidos_temp where id="'.$row["id"].'"';
		}
		if($cantidad>="1"){
			$query2='update pedidos_temp set cantidad="'.$cantidad.'" where id="'.$row["id"].'"';
		}
...
if($_POST["accion"]=="FINALIZAR"){
	...
	if(!$_POST["id_destino"]){
		Header ("location: pedidos_finaliza.php");
		exit;   	
	}
	#-------------------------------------------
...
$n_pedido=get_numero_pedido($id_sucursal);
	...
	while($row=mysql_fetch_array($result)){
		...
		$query='insert into pedidos_sucursal_estadistica_cola set 
			id_articulo="'.$row["id_articulos"].'",
