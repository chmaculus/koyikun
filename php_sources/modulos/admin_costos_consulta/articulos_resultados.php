...
echo "</tr>";
	...
	$id_articulo=$array_articulos["id"];
...
$costo_real=calcula_precio_costo($array_articulos["id"]);
//$precio_venta=calcula_precio_venta($array_articulos["id"]);
...
#---------------------------------------------------------------------------------------------
function array_costos($id_articulos){
        $query='select * from costos where id_articulos="'.$id_articulos.'"';
        $result=mysql_query($query);
        $rows=mysql_num_rows($result);
        if($rows=="1"){
