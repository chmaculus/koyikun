...
function precio_sucursal2( $id_articulo, $id_sucursal ){
	...
	if($rows<1){
		$query='select * from koyikun.precios where id_articulo="'.$id_articulo.'" and id_sucursal="1"';
	echo $query."<br>";
		$res=mysql_query($query);
		$rows=mysql_num_rows($res);
...
function verifica_tabla_precios($id_articulos, $id_sucursal ){
	$query='select * from koyikun.precios where id_articulo="'.$id_articulos.'" and id_sucursal="'.$id_sucursal.'"';
	$rows=mysql_num_rows( mysql_query($query) );
	if($rows < "1" ){
		$q='insert into koyikun.precios set id_articulo="'.$id_articulos.'", id_sucursal="'.$id_sucursal.'"';
		mysql_query($q);
		return 0;
	}
		$q='delete from koyikun.precios where id_articulo="'.$id_articulos.'", id_sucursal="'.$id_sucursal.'"';
	if($rows > 1 ){
		mysql_query($q);
		$q='insert into koyikun.precios set id_articulo="'.$id_articulos.'", id_sucursal="'.$id_sucursal.'"';
		mysql_query($q);
...
function verifica_tabla_precios3($id_articulos, $id_sucursal ){
	if($id_sucursal!=1){
		$q1='select * from koyikun.precios where id_articulo="'.$id_articulos.'" and id_sucursal="1"';
...
function insert_OR_update_precios($array_precios){
	$q1='select * from koyikun.precios where id_articulo="'.$array_precios["id_articulo"].'" and id_sucursal="'.$array_precios["id_sucursal"].'"';
	$rows=mysql_num_rows(mysql_query($q1));	
	
	$qi='insert into table koyikun.precios set ';
	$qa='update koyikun.precios set ';
	...
	if($rows>1){
		$qt='delete from koyikun.precios where id_articulo="'.$array_precios["id_articulo"].'" and id_sucursal="'.$array_precios["id_sucursal"].'"';
		mysql_query($qt);
		$query=$qi.$qii.$qc;
	}
	mysql_query($query);
