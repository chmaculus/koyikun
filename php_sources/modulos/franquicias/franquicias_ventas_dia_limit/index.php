...
echo '<br><font1>Sucursal: '.$sucursal.'</font1><br>'.chr(13);
...
echo '<td>Fecha</td><td><input type="text" name="fecha" value="'.$fecha.'"></td>';
echo '</tr>';
echo '</table>';
...
/*
$hora_desde=date("H:i:s");
$hora_hasta=date("H:i:s", time()-1800 );
*/
...
$query='select distinct numero_venta from franquicias_ventas where sucursal="'.$sucursal.'" and fecha="'.fecha_conv("/",$fecha).'"  order by fecha, hora';
...
function calcula_total_venta2($numero_venta, $sucursal){
	$query='select sum( cantidad * precio_unitario ) from franquicias_ventas where numero_venta="'.$numero_venta.'" and sucursal="'.$sucursal.'"';
	$result=mysql_query($query)or die(mysql_error());
...
function valores($numero_venta, $sucursal){
	$query='select * from franquicias_ventas where numero_venta="'.$numero_venta.'" and sucursal="'.$sucursal.'" limit 0,1';
	$result=mysql_query($query)or die(mysql_error());
...
function descuento($numero_venta, $sucursal){
	$query='select * from franquicias_ventas where numero_venta="'.$numero_venta.'" and sucursal="'.$sucursal.'" and descripcion like "Descuento%"';
	$result=mysql_query($query)or die(mysql_error());
	$rows=mysql_num_rows($result);
	if($rows>0){
		return "SI";
...
function total( $sucursal, $fecha_desde, $fecha_hasta ){
	$query='select sum( cantidad * precio_unitario ) from franquicias_ventas where sucursal="'.$sucursal.'" and fecha>="'.$fecha_desde.'" and fecha<="'.$fecha_hasta.'"';
	$result=mysql_query($query)or die(mysql_error());
