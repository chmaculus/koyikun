...
while($row=mysql_fetch_array($result)){
	...
	if( $_POST["precio_promocion"] != $array_precios["precio_base"] ){
		/* es promocion */
		$query=query_promocion($id_articulos, $row["id"], $_POST["precio_promocion"], $fecha_inicio, $fecha_finaliza, $_POST["habilitado".$row["id"]], $existe);
		mysql_query($query);
			if(mysql_error()){
				echo mysql_error();
			}
