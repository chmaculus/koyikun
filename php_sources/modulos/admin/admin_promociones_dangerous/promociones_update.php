...
while($row1=mysql_fetch_array($result1)){
	...
	$q2='select * from sucursales where id="'.$_POST["sucursal"].'"';
	echo "q: ".$q2."<br>";
	...
	$query='select * from precios where id_articulo="'.$row1["id"].'" and id_sucursal="'.$row2["id"].'"';
	$rows=mysql_num_rows( mysql_query($query) );
	echo "q3".$query." <br>rows:".$rows."<br>";
	...
	if($rows < 1 AND $row["id"] != 1 ){
		...
		if( $_POST["precio_promocion".$row["id"]] != $array_precios["precio_base"] ){
			$q0='delete from promociones where id_articulos="'.$row1["id"].'" and id_sucursal="'.$row2["id"].'"';
			mysql_query($q0);
			if(mysql_error()){
				echo mysql_error();
			}
