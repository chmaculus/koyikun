...
while($row=mysql_fetch_array($result)){
	...
	if( $_POST["margen_web".$row["id"]] != $array_costos["margen_web"] ){
		$distinto=1;
	}
/*
	if( $_POST["tarjeta".$row["id"]] != $array_costos["tarjeta"] ){
		$distinto=1;
	}
	*/
	#------------------------------

	echo "distinto: ".$distinto."<br>".chr(10);
	echo "ID: ".$row["id"]." precio_base: ".$array_precios["precio_base"]." post precio_venta: ".$_POST["precio_venta".$row["id"]]."<br>".chr(10);
	...
	if( $distinto=="1" ){
		...
		if($_POST["tipo_cambio"]=="fabrica"){
			$query='update costos set 	precio_costo="'.$_POST["precio_costo".$row["id"]].'",
											...
											margen_web="'.$_POST["margen_web".$row["id"]].'",
		...
		if ($_POST["precio_costo".$row["id"]] > 0){
			...
			$query='update precios set precio_base="'.round($precio_venta,2).'",
