...
while($row=mysql_fetch_array($result)){
	$array_marca=calcula_total_marca($vendedor, $row["marca"], $fecha_desde, $fecha_hasta);
	echo "<tr>";
	echo '<td>'.$row["marca"].'</td>';
	echo '<td>'.str_replace('.',',',$array_marca[0]).'</td>';
	echo '<td>'.$array_marca[1].'</td>';
	echo "</tr>";
}
...
function calcula_total_marca($vendedor, $marca, $fecha_desde, $fecha_hasta){
	$query='select sum( cantidad * precio_unitario ),sum(cantidad) from ventas where marca="'.$marca.'" and vendedor="'.$vendedor.'" and fecha>="'.$fecha_desde.'" and fecha<="'.$fecha_hasta.'"';
