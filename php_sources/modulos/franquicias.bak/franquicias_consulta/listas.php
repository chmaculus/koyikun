...
$query='select * from koyikun.listas where id=3 order by nombre';
...
function get_listas_porcentaje($id_articulos, $id_lista){
	$query='select * from koyikun.listas_porcentaje where id_lista="'.$id_lista.'" and id_articulos="'.$id_articulos.'"';
	$result=mysql_query($query);
	if(mysql_error()){
		echo mysql_error();
	}
