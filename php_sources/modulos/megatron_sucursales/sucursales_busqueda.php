...
<?php
include("sucursales_base.php");
?>
<body onLoad=document.aa.busqueda.focus()>

<center>
<titulo>Busqueda sucursales</titulo>
<?php
$limite=20;
$desde=$_POST["desde"];
$hasta=$_POST["hasta"];
...
$query='select * from sucursales where blabla like "%'.$_POST["busqueda"].'%"';
...
while($row=mysql_fetch_array($result)){
	...
	echo '<td><A HREF="sucursales_ingreso.php?id_sucursales='.$row["id"].'"><button>Modificar</button></A></td>';
	echo '<td><A HREF="sucursales_eliminar.php?id_sucursales='.$row["id"].'"><button>Eliminar</button></A></td>';
	echo "</tr>".chr(13);
}
echo "</table>";
