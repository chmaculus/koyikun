...
$query='select distinct marca from pedidos where finalizado="S" and fecha>="'.fecha_conv("/",$fecha_desde).'" and fecha<="'.fecha_conv("/",$fecha_hasta).'" and cajon=0 order by fecha desc';
...
while($row=mysql_fetch_array($result)){
    echo '<td>'.$row[0].'</td>';
    echo '<td><A HREF="pedidos_detalle_marca.php?marca='.$row[0].'&desde='.$fecha_desde.'&hasta='.$fecha_hasta.'"><button>Detalle</button></A></td>';
    echo "</tr>".chr(10);
}
