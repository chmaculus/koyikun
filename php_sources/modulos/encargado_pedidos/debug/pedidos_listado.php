...
$query='select distinct numero_pedido, sucursal, fecha, hora, finalizado, estado from pedidos where (estado="Pendiente" or estado is NULL ) order by sucursal,numero_pedido';
//echo "q: ".$query."<br>";
...
while($row=mysql_fetch_array($result)){
 ...
 if($zona==5){$zz="zona 5";}
    ...
    echo '<td>'.$row["hora"].'</td>';
    echo '<td>'.$estado.'</td>';
//    echo '<td>update pedidos set numero_pedido="" where sucursal="'.$row["sucursal"].'" and fecha="'.$row["fecha"].'" and numero_pedido="'.$row["numero_pedido"].'";</td>';
