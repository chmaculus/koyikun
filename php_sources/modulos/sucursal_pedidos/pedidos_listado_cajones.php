...
$query='select distinct cajon  from pedidos where estado!="Recibido" and sucursal="'.$id_sucursal.'" and cajon>0 
...
while($row=mysql_fetch_array($result)){
    ...
    echo '<td>'.$array_pedido["estado"].'</td>';
   ...
   if(file_exists ( "./images/".$array_pedido["estado"].".jpg" )==1){
            echo '<td><img  src="./images/'.$array_pedido["estado"].'".jpg" width="50" height="40"></td>';
    }else{
                echo '<td></td>';
    }
...
function contenido_cajon($numero_cajon, $id_sucursal){
		$query='select  marca,count(*) as count from pedidos where cajon="'.$numero_cajon.'" and finalizado="S" and estado!="Recibido" and fecha>="2020-08-31" group by marca';
...
function trae_datos($numero_cajon, $id_sucursal){
	$q='select * from pedidos where cajon="'.$numero_cajon.'" and finalizado="S" and estado!="Recibido" and sucursal="'.$id_sucursal.'" and cajon>0 and fecha>="2020-08-31" limit 0,1';
...
function pedidos_cajon($numero_cajon, $id_sucursal){
	$q='select distinct numero_pedido from pedidos where cajon="'.$numero_cajon.'" and sucursal="'.$id_sucursal.'" and estado!="Recibido" and fecha>="2020-08-31"';
	$res=mysql_query($q);
