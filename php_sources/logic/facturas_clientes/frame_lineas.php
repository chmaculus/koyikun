...
if ($modif<>1) {
		...
		if ($retorno==0) {	
				...
				$vto=$_POST["vencimiento"];
				$lote=$_POST["lote"];
				
				
//				$sel_insert="INSERT INTO factulineatmp 
//				(codfactura,		numlinea,	codigo,			codfamilia,		cantidad,	precio,		importe,	dcto) VALUES 
//				('$codfacturatmp',	'',			'$codarticulo',	'$codfamilia',	'$cantidad','$precio',	'$importe',	'$descuento')";

				$sel_insert='insert into logic.factulineatmp set codfactura="'.$codfacturatmp.'", numlinea="", codfamilia="'.$codfamilia.'", codigo="'.$codarticulo.'", cantidad="'.$cantidad.'", precio="'.$precio.'", importe="'.$importe.'", dcto="'.$descuento.'", vto="'.$vto.'", lote="'.$lote.'"';
...
//echo "rows: ".mysql_num_rows($rs_lineas)."<br>";
...
for ($i = 0; $i < mysql_num_rows($rs_lineas); $i++) {
	...
	$vencimiento=mysql_result($rs_lineas,$i,"vto");
	$lote=mysql_result($rs_lineas,$i,"lote");
	if ($i % 2) { $fondolinea="itemParTabla"; } else { $fondolinea="itemImparTabla"; } ?>
			<tr class="<? echo $fondolinea?>">
				<td width="5%"><? echo $i+1?></td>
				<td width="18%"><? echo $referencia?></td>
				<td width="41%"><? echo $descripcion?></td>
				<td width="8%" class="aCentro"><? echo $vencimiento?></td>
				<td width="8%" class="aCentro"><? echo $lote?></td>
