...
if ($accion=="alta") {
	$query_operacion="INSERT INTO presupuestos (codalbaran, codfactura, fecha, iva, codcliente, estado, borrado) VALUES ('', '0', '$fecha', '$iva', '$codcliente', '1', '0')";					
	$rs_operacion=mysql_query($query_operacion);
	$codalbaran=mysql_insert_id();
	if ($rs_operacion) { $mensaje="El albar&aacute;n ha sido dado de alta correctamente"; }
	$query_tmp="SELECT * FROM presupuestoslineatmp WHERE codalbaran='$codalbarantmp' ORDER BY numlinea ASC";
	...
	while ($contador < mysql_num_rows($rs_tmp)) {
		...
		$sel_insertar="INSERT INTO presupuestoslinea (codalbaran,numlinea,codfamilia,codigo,cantidad,precio,importe,dcto) VALUES 
	...
	$sel_act="UPDATE presupuestos SET totalalbaran='$preciototal' WHERE codalbaran='$codalbaran'";
...
if ($accion=="modificar") {
	...
	$act_albaran="UPDATE presupuestos SET codcliente='$codcliente', fecha='$fecha', iva='$iva' WHERE codalbaran='$codalbaran'";
	$rs_albaran=mysql_query($act_albaran);
	$sel_lineas = "SELECT codigo,codfamilia,cantidad FROM presupuestoslinea WHERE codalbaran='$codalbaran' order by numlinea";
	...
	$sel_borrar = "DELETE FROM presupuestoslinea WHERE codalbaran='$codalbaran'";
	$rs_borrar = mysql_query($sel_borrar);
	$sel_lineastmp = "SELECT * FROM presupuestoslineatmp WHERE codalbaran='$codalbarantmp' ORDER BY numlinea";
	...
	while ($contador < mysql_num_rows($rs_lineastmp)) {
		...
		$sel_insert = "INSERT INTO presupuestoslinea (codalbaran,numlinea,codigo,codfamilia,cantidad,precio,importe,dcto) 
...
if ($accion=="baja") {
	...
	$query="UPDATE presupuestos SET borrado=1 WHERE codalbaran='$codalbaran'";
	$rs_query=mysql_query($query);
	$query="SELECT * FROM presupuestoslinea WHERE codalbaran='$codalbaran' ORDER BY numlinea ASC";
	...
	$query_mostrar="SELECT * FROM presupuestos WHERE codalbaran='$codalbaran'";
...
if ($accion=="convertir") {
	...
	$fecha=$_POST["fecha"];
	$fecha=explota($fecha);
	$sel_albaran="SELECT * FROM presupuestos WHERE codalbaran='$codalbaran'";
	...
	$act_albaran="UPDATE presupuestos SET codfactura='$codfactura',estado='2' WHERE codalbaran='$codalbaran'";
	$rs_act=mysql_query($act_albaran);
	$sel_lineas="SELECT * FROM presupuestoslinea WHERE codalbaran='$codalbaran' ORDER BY numlinea ASC";
	$rs_lineas=mysql_query($sel_lineas);
	$contador=0;
	while ($contador < mysql_num_rows($rs_lineas)) {
		$codfamilia=mysql_result($rs_lineas,$contador,"codfamilia");
		...
		<div id="pagina">
			<div id="zonaContenido">
				...
				<div id="frmBusqueda">
					...
					<table class="fuente8" width="98%" cellspacing=0 cellpadding=3 border=0 ID="Table1">
					  <? $sel_lineas="SELECT presupuestoslinea.*,articulos.*,familias.nombre as nombrefamilia FROM presupuestoslinea,articulos,familias WHERE presupuestoslinea.codalbaran='$codalbaran' AND presupuestoslinea.codigo=articulos.codarticulo AND presupuestoslinea.codfamilia=articulos.codfamilia AND articulos.codfamilia=familias.codfamilia ORDER BY presupuestoslinea.numlinea ASC";
$rs_lineas=mysql_query($sel_lineas);
