	...
	<head>
		...
		function modificar_albaran(codalbaran,codproveedor,marcaestado) {
			if (marcaestado==1) {
				...
				alert ("No se puede modificar un remito ya facturado.");
			}
		}
		...
		function convertir_albaran(codalbaran,codproveedor,marcaestado) {
			if (marcaestado==1) {
				...
				alert ("No se puede convertir un remito ya facturado.");
			}
		}
			...
			<input type="hidden" name="numfilas" id="numfilas" value="<? echo $filas?>">
				...
				if ($iniciopagina>$filas) { $iniciopagina=0; }
					...
					<table class="fuente8" width="87%" cellspacing=0 cellpadding=3 border=0>
						<tr>
							<td width="100%" class="mensaje"><?php echo "No hay ning&uacute; remito que cumpla con los criterios de b&uacute;squeda";?></td>
					    </tr>
					</table>					
					<? } ?>	
				</div>				
