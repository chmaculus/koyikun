...
$q='select id, sucursal from pedidos where numero_pedido is null order by sucursal';
$res=mysql_query($q);
$suc=0;
...
while($row=mysql_fetch_array($res)){
        if($suc!=$row["sucursal"]){
                $count1=30;
                $suc=$row["sucursal"];
                incrementa_n_pedido($row["sucursal"]);
                $npedid=get_numero_pedido($row["sucursal"]);
        }
        if($count1>=30){
...
if($exec==1){
	mysql_query('update pedidos set zona=1 where zona=0 or zona is null');
}
