...
#-------------------------------------------------------------------------------
#trae acumulado mes
$q='select sum(cantidad * Precio_unitario) from ventas where fecha>="'.$mes_anio.'-01" and fecha<="'.$mes_anio.'-31"  and sucursal !="157 AP OnLine" and sucursal!="88"';
$res=mysql_query($q);
...
$mes_ant=($mes-1);
if($mes==0){$mes=12;}
$q='select sum(cantidad * Precio_unitario) from ventas where fecha>="'.$anio.'-'.$mes_ant.'-01" and fecha<="'.$anio.'-'.$mes_ant.'-31"  and sucursal !="157 AP OnLine" and sucursal!="88"';
$res=mysql_query($q);
...
function acumulado_menos_un_anio( $fecha, $sucursal ){
        ...
        $bb=$aa[0]."-".( $mes);
        $q='select sum( cantidad * precio_unitario ) from ventas where fecha >= "'.$bb.'-01" and fecha <= "'.$bb.'-31"  and sucursal !="157 AP OnLine" and sucursal!="88"';
