...
$mes=$aa[1];
$bb=$aa[0]."-".$mes;
$q='select sum( cantidad * precio_unitario ) from ventas where fecha >= "'.$bb.'-01" and fecha <= "'.$bb.'-31" and sucursal="'.$row["sucursal"].'" and tipo_pago="co"';
//$q='select sum(cantidad * precio_unitario) from ventas where fecha="'.$fecha.'" and sucursal="'.$row["sucursal"].'" and tipo_pago="co"'; 
$rese=mysql_query($q);
$array=mysql_fetch_array($rese);
echo "<tr>";
echo '<td>ACT E</td><td>'.$array[0].'</td>';
echo '<td>'.round((( $array[0] * 100 ) / $acum_mes),1).'%</td>';
echo "</tr>";
...
$mes=$aa[1];
$bb=$aa[0]."-".$mes;
$q='select sum( cantidad * precio_unitario ) from ventas where fecha >= "'.$bb.'-01" and fecha <= "'.$bb.'-31" and sucursal="'.$row["sucursal"].'" and tipo_pago="de"';
$rese=mysql_query($q);
$array=mysql_fetch_array($rese);
$actdebito=$array[0];
echo "<tr>";
echo '<td>ACT D</td><td>'.$array[0].'</td>';
echo '<td>'.round((( $array[0] * 100 ) / $acum_mes),1).'%</td>';
echo "</tr>";
#------------------------------------------------------
...
$mes=$aa[1];
$bb=$aa[0]."-".$mes;
$q='select sum( cantidad * precio_unitario ) from ventas where fecha >= "'.$bb.'-01" and fecha <= "'.$bb.'-31" and sucursal="'.$row["sucursal"].'" and tipo_pago="ta"';
$rese=mysql_query($q);
$array=mysql_fetch_array($rese);
$acttarjeta=$array[0];
echo "<tr>";
echo '<td>ACT T</td><td>'.$array[0].'</td>';
echo '<td>'.round((( $array[0] * 100 ) / $acum_mes),1).'%</td>';
echo "</tr>";
#------------------------------------------------------
...
#ACT D+T
echo "<tr>";
echo '<td>ACT D+T</td><td>'.($actdebito + $acttarjeta).'</td>';
echo '<td>'.round((( ($actdebito + $acttarjeta) * 100 ) / $acum_mes),1).'%</td>';
echo "</tr>";
#------------------------------------------------------
